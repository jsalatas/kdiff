...
{
//  qCDebug() << "Width = " << m_width << ", height = " << m_height << ", posX = " << posX << ", posY = " << posY;
...
// Patch based on work by Felix Berger as put as attachment to bug 75794
    ...
    while (x > 0 && y > 0) {
        ...
        switch (direction) {
            case 0: // north
//          qCDebug() << "Picking north";
//          qCDebug() << "Source[" << ( x - 1 ) << "] = " << QString( source[ x-1 ] ) << ", destination[" << ( y - 1 ) << "] = " << QString( destination[ y-1 ] );
...
//              qCDebug() << "CurrentValue: " << currentValue;
                    if(n == currentValue) {
...
//          qCDebug() << "Source[" << ( x - 1 ) << "] = " << QString( source[ x-1 ] ) << ", destination[" << ( y - 1 ) << "] = " << QString( destination[ y-1 ] );
                ...
                if(c && c->type() == Marker::End) {
//              qCDebug() << "End found: CurrentValue: " << currentValue;
                    if(nw == currentValue) {
                        m_sequences->prependSecond(new Marker(Marker::Start, y));
                    }
// else: the change continues, do not do anything
...
//  {
//      c = *mit;
//      qCDebug() << "Source Marker Entry : Type: " << c->type() << ", Offset: " << c->offset();
...
//  }
//  qCDebug() << "Source Resulting stringlist : " << list.join("\n");
//
//  list.clear();
//  prevValue = 0;
//
//  qCDebug() << "Destination string: " << destination;
//  mit = m_sequences->markerListSecond().begin();
//  end = m_sequences->markerListSecond().end();
//  for ( ; mit != end; ++mit )
//  {
//      c = *mit;
//      qCDebug() << "Destination Marker Entry : Type: " << c->type() << ", Offset: " << c->offset();
...
//  }
//  qCDebug() << "Destination Resulting string : " << list.join("\n");
}
