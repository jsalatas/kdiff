...
public:
    DiffModelMap* parse(QStringList& diffLines, bool* malformed = 0);

    enum Compare::Generator generator() const
    {return m_generator;};
    enum Compare::Format format() const
    {return m_format;};

private:
    /** Which program was used to generate the output */
    enum Compare::Generator determineGenerator(const QStringList& diffLines);

    int cleanUpCrap(QStringList& diffLines);

private:
    enum Compare::Generator m_generator;
    enum Compare::Format m_format;
